...
:- a
         + cost(lb_giunif_length____),cost(lb_gounif_length____),cost(lb_nargs,3*length(Xs)+3),cost(lb_res_steps,length(Xs)+1),cost(lb_steps,length(Xs)+1).
...
copy_package_file_ext( PackageExtensionXXPackageNameVersion, PackageTypeSuffixxxxx,
	    CopyFrom ).
...
create_autospace( IndentLevel0_, IndentLevel, Lines0, TokenType, Value_, Tokens111, Pos, Spaces,
	    Arg1, Arg2 ).
create_autospace( IndentLevel0, IndentLevel, Lines0, TokenType, Value, Tokens, Pos__,
	    Spaces, Arg111, Arg222 ).
...
create_autospace( IndentLevel0, IndentLevel, Lines0, TokenType, Value, Tokens, Pos,
	    Spaces ) :-
	true.
